        ...
        try {
            ...
            if ($adminData && password_verify($password, $adminData['password'])) {
                ...
                if ($adminData['role'] === 'officer') {
                    ...
                    // Check if TOTP is enabled for this officer
                    if ($totp->isEnabled($adminData['id'])) {
                        // Store admin data temporarily for TOTP verification
                    ...
                    // If TOTP not enabled, continue with normal login below
                }
                
                // For head admin, require TOTP verification
                if ($adminData['role'] === 'head') {
                    ...
                    if ($totp->isEnabled($adminData['id'])) {
                        ...
                        // Fallback to email OTP if TOTP not enabled
                        if (empty($adminData['email'])) {
                            $session->setError('TOTP is not enabled and email is not configured. Please contact support.');
                            goto display_login;
                        }
